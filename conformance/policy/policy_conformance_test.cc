...
ABSL_FLAG(std::string, testdata_dir, "", "Path to testdata directory.");
...
class InputEvaluator {
 public:
  ...
  absl::StatusOr<cel::Value> Evaluate(absl::string_view expr_str,
...
absl::StatusOr<cel::Value> EvaluateInputValue(
  ...
  if (input_val.has_expr()) {
    return evaluator.Evaluate(input_val.expr(), arena);
  }
...
absl::StatusOr<testing::Matcher<cel::Value>> MakeExpectedValueMatcher(
  ...
  if (output.has_result_expr()) {
    CEL_ASSIGN_OR_RETURN(expected_val,
                         input_evaluator.Evaluate(output.result_expr(), arena));
 ...
 public:
  ...
  void RunTest(const cel::expr::conformance::test::TestCase& test,
    ...
    for (const auto& [var_name, input_val] : test.input()) {
      auto val_or = EvaluateInputValue(
          input_val, *input_evaluator_, pool_.get(),
          google::protobuf::MessageFactory::generated_factory(), &arena);
      ASSERT_THAT(val_or.status(), IsOk())
          << "Failed to evaluate input '" << var_name << "' in "
          << full_test_name;
      activation.InsertOrAssignValue(var_name, *std::move(val_or));
    }
    ...
    ASSERT_OK_AND_ASSIGN(
        auto matcher, MakeExpectedValueMatcher(
                          output, *input_evaluator_, pool_.get(),
                          google::protobuf::MessageFactory::generated_factory(), &arena));
...
absl::Status RegisterTestSuite(
  ...
  auto runner = std::make_shared<PolicyTestSuiteRunner>(
      ...
      input_evaluator, absl::StrContains(suite_name, "compile_errors"));
...
void RegisterAllTests() {
  std::string testdata_dir_flag = absl::GetFlag(FLAGS_testdata_dir);
  std::vector<std::string> skip_tests = absl::GetFlag(FLAGS_skip_tests);

  std::string abs_testdata_dir =
      cel::internal::ResolveRunfilesPath(testdata_dir_flag);
  ABSL_CHECK(!abs_testdata_dir.empty())
      << "Could not find testdata directory: " << testdata_dir_flag;

  auto evaluator_or = InputEvaluator::Create(GetSharedTestingDescriptorPool());
  ...
  std::filesystem::path testdata_path(abs_testdata_dir);
  ...
  for (const auto& entry :
    ...
    if (std::filesystem::exists(dir_path / "policy.yaml") &&
      ...
      ABSL_CHECK_OK(RegisterTestSuite(dir_path, suite_name, evaluator,
