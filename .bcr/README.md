...
This directory contains templates used by the [Publish to BCR](https://github.com/bazel-contrib/publish-to-bcr) GitHub Action to automatically publish new versions of cel-cpp to the [Bazel Central Registry (BCR)](https://github.com/bazelbuild/bazel-central-registry).
...
- `{REPO}`: Repository name (cel-cpp)
- `{VERSION}`: Version number extracted from the tag (e.g., `0.14.0` from `v0.14.0`)
- `{TAG}`: Full tag name (e.g., `v0.14.0`)
