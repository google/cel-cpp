...
absl::Status TypeCheckerBuilder::AddLibrary(CheckerLibrary library) {
  if (!library.id.empty() && !library_ids_.insert(library.id).second) {
    return absl::AlreadyExistsError(
        absl::StrCat("library '", library.id, "' already exists unique"));
  }
...
absl::Status TypeCheckerBuilder::AddVariable(VariableDecl decl) {
  bool inserted = env_.InsertVariableIfAbsent(std::move(decl));
  if (!inserted) {
    return absl::AlreadyExistsError(
...
absl::Status TypeCheckerBuilder::AddFunction(FunctionDecl decl) {
  bool inserted = env_.InsertFunctionIfAbsent(std::move(decl));
  if (!inserted) {
    return absl::AlreadyExistsError(
