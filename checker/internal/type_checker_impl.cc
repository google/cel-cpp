...
class ResolveVisitor : public AstVisitorBase {
 public:
  ...
  void PreVisitComprehension(const Expr& expr,
    ...
    auto accu_scope =
        std::make_unique<VariableScope>(current_scope_->MakeNestedScope());
    const auto* accu_scope_ptr = accu_scope.get();

    auto iter_scope =
        std::make_unique<VariableScope>(accu_scope->MakeNestedScope());
  ...
  void PostVisitStruct(const Expr& expr,
    ...
    std::cerr << "resolving struct: " << create_struct.name() << "\n";
    namespace_generator_.GenerateCandidates(
        create_struct.name(),
        [&](absl::string_view candidate) { return true; });
  }
  ...
  void ResolveSimpleIdentifier(const Expr& expr, absl::string_view name) {
    ...
    namespace_generator_.GenerateCandidates(
        name, [&decl, this](absl::string_view candidate) {
          std::cerr << "simple candidate: " << candidate << "\n";
...
void ResolveVisitor::PostVisitIdent(const Expr& expr, const IdentExpr& ident) {
  ...
  while (stack_pos > 0) {
    const Expr* parent = expr_stack_[stack_pos - 1];
    const Expr* self = expr_stack_[stack_pos];
    --stack_pos;
    if (parent->has_select_expr()) {
      qualifiers.push_back(parent->select_expr().field());
      continue;
    }
    if (parent->has_call_expr() && (&parent->call_expr().target() == self)) {
  ...
  if (receiver_call == nullptr) {
    ResolveQualifiedIdentifier(*expr_stack_[stack_pos], qualifiers);
  } else {
    std::cerr << "Need to resolve fn: " << FormatCandidate(qualifiers) << "\n";
...
void ResolveVisitor::PostVisitCall(const Expr& expr, const CallExpr& call) {
  if (auto iter = maybe_namespaced_functions_.find(&expr);
    ...
    std::cerr << "resolving namespaced function: "
              << FormatCandidate(iter->second) << "." << call.function()
              << "\n";
...
const FunctionDecl* ResolveVisitor::ResolveFunctionCall(
  ...
  namespace_generator_.GenerateCandidates(
      function_name, [&, this](absl::string_view candidate) -> bool {
        std::cerr << "searching for call: " << candidate << " args "
                  << arg_count << "\n";
...
void ResolveVisitor::ResolveQualifiedIdentifier(
  ...
  std::cerr << "resolving qualified identifier: " << FormatCandidate(qualifiers)
            << "\n";
  ...
  namespace_generator_.GenerateCandidates(
      qualifiers, [&decl, &segment_index_out, this](absl::string_view candidate,
                                                    int segment_index) {
        std::cerr << "qualified candidate: " << candidate << " "
                  << segment_index << "\n";
        decl = current_scope_->LookupVariable(candidate);
        if (decl != nullptr) {
