...
std::string SevString(IssueSeverity severity) {
  switch (severity) {
    case IssueSeverity::kDeprecated:
      return "Deprecated";
    case IssueSeverity::kError:
      return "Error";
    case IssueSeverity::kWarning:
      return "Warning";
    case IssueSeverity::kInformation:
      return "Information";
  }
}
...
absl::StatusOr<std::unique_ptr<Ast>> MakeParsedAst(
    absl::string_view expression) {
  CEL_ASSIGN_OR_RETURN(auto parsed,
                       google::api::expr::parser::Parse(expression));

  return cel::extensions::CreateAstFromParsedExpr(std::move(parsed));
}
...
TEST_F(TypeCheckerImplTest, SmokeTest) {
  ...
  ASSERT_OK(RegisterMinimalBuiltins(env));

  TypeCheckerImpl impl(std::move(env));
  ASSERT_OK_AND_ASSIGN(auto ast, MakeParsedAst("1 + 2"));
...
TEST_F(TypeCheckerImplTest, SimpleIdentsResolved) {
  ...
  TypeCheckerImpl impl(std::move(env));
  ASSERT_OK_AND_ASSIGN(auto ast, MakeParsedAst("x + y"));
  ASSERT_OK_AND_ASSIGN(ValidationResult result, impl.Check(std::move(ast)));
...
TEST_F(TypeCheckerImplTest, ReportMissingIdentDecl) {
  ...
  EXPECT_THAT(result.GetIssues(),
              ElementsAre(IsIssueWithSubstring(IssueSeverity::kError,
...
TEST_F(TypeCheckerImplTest, QualifiedIdentsResolved) {
  ...
  TypeCheckerImpl impl(std::move(env));
  ASSERT_OK_AND_ASSIGN(auto ast, MakeParsedAst("x.y + x.z"));
  ASSERT_OK_AND_ASSIGN(ValidationResult result, impl.Check(std::move(ast)));
...
TEST_F(TypeCheckerImplTest, MemberFunctionCallResolved) {
  ...
  FunctionDecl foo;
  foo.set_name("foo");
  ASSERT_OK(foo.AddOverload(MakeMemberOverloadDecl("int_foo_int",
                                                   /*return_type=*/IntType(),
                                                   IntType(), IntType())));
  env.InsertFunctionIfAbsent(std::move(foo));

  TypeCheckerImpl impl(std::move(env));
  ASSERT_OK_AND_ASSIGN(auto ast, MakeParsedAst("x.foo(y)"));
  ASSERT_OK_AND_ASSIGN(ValidationResult result, impl.Check(std::move(ast)));
...
TEST_F(TypeCheckerImplTest, MemberFunctionCallNotDeclared) {
  ...
  EXPECT_THAT(result.GetIssues(),
              ElementsAre(IsIssueWithSubstring(
                  IssueSeverity::kError, "undeclared reference to 'foo'")));
}
...
TEST_F(TypeCheckerImplTest, ComprehensionVariablesResolved) {
  ...
  ASSERT_OK_AND_ASSIGN(auto ast,
                       MakeParsedAst("[1, 2, 3].exists(x, x * x > 10)"));
  ASSERT_OK_AND_ASSIGN(ValidationResult result, impl.Check(std::move(ast)));
...
TEST_F(TypeCheckerImplTest, NestedComprehensions) {
  ...
  ASSERT_OK_AND_ASSIGN(
      auto ast,
      MakeParsedAst("[1, 2].all(x, ['1', '2'].exists(y, int(y) == x))"));
  ASSERT_OK_AND_ASSIGN(ValidationResult result, impl.Check(std::move(ast)));
...
TEST_F(TypeCheckerImplTest, ComprehensionVarsFollowNamespacePriorityRules) {
  ...
  TypeCheckerImpl impl(std::move(env));
  ASSERT_OK_AND_ASSIGN(auto ast, MakeParsedAst("['1', '2'].all(x, x == 2)"));
  ASSERT_OK_AND_ASSIGN(ValidationResult result, impl.Check(std::move(ast)));
