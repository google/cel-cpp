...
class VariableScope {
 public:
  explicit VariableScope(const TypeCheckEnv& env)
      : env_(&env), parent_(nullptr) {}
  explicit VariableScope(const TypeCheckEnv& env, const VariableScope* parent)
  ...
  VariableScope MakeNestedScope() const { return VariableScope(*env_, this); }
