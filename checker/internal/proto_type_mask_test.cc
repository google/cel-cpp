...
TEST(ProtoTypeMaskTest, AbslStringifyPrintsTypeNameAndFieldPaths) {
  ...
  ProtoTypeMask proto_type_mask(type_name, field_paths);
  EXPECT_THAT(
      absl::StrCat(proto_type_mask),
      AllOf(
          HasSubstr(
              "ProtoTypeMask { type name: 'google.type.Expr', field paths: {"),
          HasSubstr("FieldPath { field path: 'resource.name', field selection: "
                    "{'resource', 'name'} }"),
          HasSubstr("FieldPath { field path: 'resource.type', field selection: "
                    "{'resource', 'type'} }")));
}
