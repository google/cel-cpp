...
TEST(ProtoTypeMaskRegistryTest, CreateWithDelimiterFieldPathReturnsError) {
  std::vector<ProtoTypeMask> proto_type_masks = {
      ProtoTypeMask("cel.expr.conformance.proto3.TestAllTypes", {"."})};
  EXPECT_THAT(
      ProtoTypeMaskRegistry::Create(GetSharedTestingDescriptorPool().get(),
...
TEST(ProtoTypeMaskRegistryTest,
     CreateWithDepthTwoMessageUnknownFieldReturnsError) {
  std::vector<ProtoTypeMask> proto_type_masks = {
      ProtoTypeMask("cel.expr.conformance.proto3.TestAllTypes",
...
TEST(ProtoTypeMaskRegistryTest,
     CreateWithDepthOneRepeatedMessageFieldSucceedsAndFieldsAreVisible) {
  std::vector<ProtoTypeMask> proto_type_masks = {ProtoTypeMask(
  ...
  EXPECT_THAT(proto_type_mask_registry->GetTypesAndVisibleFields(),
              UnorderedElementsAre(
                  Pair("cel.expr.conformance.proto3.TestAllTypes",
                       UnorderedElementsAre("repeated_nested_message"))));
  EXPECT_TRUE(proto_type_mask_registry->FieldIsVisible("any_type_name",
                                                       "any_field_name"));
  EXPECT_TRUE(proto_type_mask_registry->FieldIsVisible(
      "cel.expr.conformance.proto3.TestAllTypes", "repeated_nested_message"));
...
TEST(ProtoTypeMaskRegistryTest,
     CreateWithDepthTwoRepeatedMessageFieldReturnsError) {
  std::vector<ProtoTypeMask> proto_type_masks = {
      ProtoTypeMask("cel.expr.conformance.proto3.TestAllTypes",
  ...
  EXPECT_THAT(
      ...
      StatusIs(
          absl::StatusCode::kInvalidArgument,
          HasSubstr("field 'repeated_nested_message' is not a message type")));
}
...
TEST(ProtoTypeMaskRegistryTest, AbslStringifyPrintsTypesAndVisibleFieldsMap) {
  std::vector<ProtoTypeMask> proto_type_masks = {ProtoTypeMask(
      "cel.expr.conformance.proto3.TestAllTypes", {"standalone_message.bb"})};
  ASSERT_OK_AND_ASSIGN(
      std::shared_ptr<ProtoTypeMaskRegistry> proto_type_mask_registry,
      ProtoTypeMaskRegistry::Create(GetSharedTestingDescriptorPool().get(),
                                    proto_type_masks));
  EXPECT_THAT(
      absl::StrCat(proto_type_mask_registry),
      AllOf(HasSubstr("ProtoTypeMaskRegistry {"),
            HasSubstr("{type: 'cel.expr.conformance.proto3.TestAllTypes', "
                      "visible_fields: 'standalone_message'}"),
            HasSubstr("{type: "
                      "'cel.expr.conformance.proto3.TestAllTypes.NestedMessage'"
                      ", visible_fields: 'bb'}")));
}
