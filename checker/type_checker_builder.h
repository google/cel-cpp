...
using CheckerOptions = absl::FunctionRef<absl::Status(TypeCheckerBuilder&)>;

struct CheckerLibrary {
  // Optional identifier to avoid collisions.
  // If id is empty, it is not considered.
  std::string id;
  // Functional implementation applying the library to the type checker.
...
class TypeCheckerBuilder {
 public:
  ...
  absl::Status AddVariable(VariableDecl decl);
  absl::Status AddFunction(FunctionDecl decl);
