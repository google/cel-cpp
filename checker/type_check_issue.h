...
enum class IssueSeverity { kError, kWarning, kInformation, kDeprecated };

// Represents a single issue identified in type checking.
class TypeCheckIssue {
 public:
  TypeCheckIssue(IssueSeverity severity, SourceLocation location,
                 std::string message)
      : severity_(severity),
        location_(location),
  ...
  static TypeCheckIssue CreateError(SourceLocation location,
                                    std::string message) {
    return TypeCheckIssue(IssueSeverity::kError, location, std::move(message));
  }

  // Factory for error-severity issues.
  // line is 1-based, column is 0-based.
  static TypeCheckIssue CreateError(int line, int column, std::string message) {
    return TypeCheckIssue(IssueSeverity::kError, SourceLocation{line, column},
                          std::move(message));
  }
  ...
  IssueSeverity severity() const { return severity_; }
