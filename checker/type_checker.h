...
class TypeChecker {
 public:
  ...
  //
  // Most errors are returned as Issues in the validation result. A non-ok
  // status is returned if type checking can't reasonably complete (e.g.
  // extension returns a Status).
  virtual absl::StatusOr<ValidationResult> Check(
      std::unique_ptr<Ast> ast) const = 0;

  // TOOD(b/): overload for cref AST.
};
