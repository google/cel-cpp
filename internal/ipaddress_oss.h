...
#ifndef THIRD_PARTY_CEL_CPP_INTERNAL_IPADDRESS_OSS_H_
#define THIRD_PARTY_CEL_CPP_INTERNAL_IPADDRESS_OSS_H_
...
class IPAddress {
 public:
  ...
  friend H AbslHashValue(H h, const IPAddress& ip) {
    switch (ip.address_family()) {
      case AF_INET:
        return H::combine(std::move(h), ip.address_family(),
                          ip.addr_.addr4.s_addr);
      case AF_INET6:
        return H::combine(
            std::move(h), ip.address_family(), ip.addr_.addr6.s6_addr32[0],
            ip.addr_.addr6.s6_addr32[1], ip.addr_.addr6.s6_addr32[2],
            ip.addr_.addr6.s6_addr32[3]);
      default:
        return H::combine(std::move(h), ip.address_family());
    }
  }
 ...
 private:
  ...
  bool Equals6(const IPAddress& other) const {
    ...
    // These 64-bit CPUs have efficient implementations of
    // UnalignedLoad<uint64_t>().
    uint64_t a1 =
        ipaddress_internal::UnalignedLoad<uint64_t>(&addr_.addr6.s6_addr32[0]);
    uint64_t a2 =
        ipaddress_internal::UnalignedLoad<uint64_t>(&addr_.addr6.s6_addr32[2]);
    uint64_t b1 = ipaddress_internal::UnalignedLoad<uint64_t>(
        &other.addr_.addr6.s6_addr32[0]);
    uint64_t b2 = ipaddress_internal::UnalignedLoad<uint64_t>(
        &other.addr_.addr6.s6_addr32[2]);
...
inline IPRange StringToIPRangeAndTruncateOrDie(absl::string_view str) {
  IPRange ipr;
  ABSL_CHECK(StringToIPRangeAndTruncate(str, &ipr))
      << "Invalid IP range " << str;
  return ipr;
}
...
// Returns true if ip is initialized.
inline bool IsInitializedAddress(const IPAddress& ip) {
  return ip.address_family() != AF_UNSPEC;
}

}  // namespace cel::internal

#endif  // THIRD_PARTY_CEL_CPP_COMMON_IPADDRESS_OSS_H_
