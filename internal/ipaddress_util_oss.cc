...
inline uint32_t UnalignedLoad32(const void* p) {
  return ipaddress_internal::UnalignedLoad<uint32_t>(p);
}
inline uint64_t UnalignedLoad64(const void* p) {
  return ipaddress_internal::UnalignedLoad<uint64_t>(p);
}
}  // namespace
...
bool GetMappedIPv4Address(const IPAddress& ip6, IPAddress* ip4) {
  ...
  if (UnalignedLoad64(addr6.s6_addr16) != 0 || addr6.s6_addr16[4] != 0 ||
  ...
  if (ip4) {
    in_addr ipv4;
    ipv4.s_addr = UnalignedLoad32(addr6.s6_addr16 + 6);
    *ip4 = IPAddress(ipv4);
  }
