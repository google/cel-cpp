...
#define UNALIGNED_LOAD32(p) cel::common_internal::UnalignedLoad<uint32_t>(p)
#define UNALIGNED_LOAD64(p) cel::common_internal::UnalignedLoad<uint64_t>(p)
...
IPAddress IPAddress::Loopback4() { return HostUInt32ToIPAddress(0x7f000001); }
...
IPAddress UInt128ToIPAddress(const absl::uint128 bigint) {
  in6_addr addr6;
  addr6.s6_addr32[0] =
      htonl(static_cast<uint32_t>(absl::Uint128High64(bigint) >> 32));
  addr6.s6_addr32[1] =
      htonl(static_cast<uint32_t>(absl::Uint128High64(bigint) & 0xFFFFFFFFULL));
  addr6.s6_addr32[2] =
      htonl(static_cast<uint32_t>(absl::Uint128Low64(bigint) >> 32));
  addr6.s6_addr32[3] =
      htonl(static_cast<uint32_t>(absl::Uint128Low64(bigint) & 0xFFFFFFFFULL));
...
void AppendIPv6ToString(const in6_addr& addr, std::string* out) {
  if (addr.s6_addr32[0] == 0 && addr.s6_addr32[1] == 0) {
    ...
    if (i < 8) {
      for (; i < 8; ++i) {
        absl::StrAppend(out, ":", absl::Hex(ntohs(addr.s6_addr16[i])));
      }
    } else {
      out->push_back(':');
    }
  ...
  switch (address_family_) {
    ...
    case AF_INET6:
      if (ABSL_PREDICT_FALSE(HasCompactScopeId(addr_.addr6))) {
        ...
        // is no inherent guarantee that a given interface index remains
        // valid. For now, output them the same way as their unscoped cousins
        // -- what to do with the interface index and/or name is likely to be
        // an application-dependent matter.
        ABSL_VLOG(2) << "ToPackedString() dropping scope ID";
...
absl::Status InternalGetaddrinfoErrorToStatus(int rval, int copied_errno) {
  ...
  // Note that getaddrinfo is only guaranteed to set errno when the return
  // value is EAI_SYSTEM.  Otherwise, errno is unreliable.
  //
  ...
  if (host_address_family == AF_INET &&
    ...
    if (mask.s_addr == 0) {
      *out = 0;
      ...
      uint32_t inv_mask = (~ntohl(mask.s_addr)) + 1;
      // Power of two iff x & (x - 1) == 0.
      if ((inv_mask & (inv_mask - 1)) != 0) {
        return false;
      }
      *out = 32 - __builtin_ffs(ntohl(mask.s_addr)) + 1;
    }
    return true;
  }
...
bool InternalStringToIPRange(absl::string_view str,
  ...
  if (slash_pos != absl::string_view::npos) {
    int length;
    if (!InternalStringToNetmaskLength(absl::ClippedSubstr(str, slash_pos + 1),
                                       out->first.address_family(), &length)) {
      return false;
    }
    out->second = length;
    return true;
...
//-----------------------------------------------------------------------------
// Carry over required functions from ipaddress.cc
//------------------------------------------------------------------------------
