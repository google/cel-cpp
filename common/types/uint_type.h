...
inline constexpr bool operator==(const UintType&, const UintType&) {
  return true;
}

inline constexpr bool operator!=(const UintType& lhs, const UintType& rhs) {
  return !operator==(lhs, rhs);
}

template <typename H>
H AbslHashValue(H state, const UintType& type) {
...
template <typename H>
H AbslHashValue(H state, UintTypeView type) {
  return H::combine(std::move(state), type.kind());
}
