...
inline constexpr bool operator==(const BoolWrapperType&,
                                 const BoolWrapperType&) {
  return true;
}

inline constexpr bool operator!=(const BoolWrapperType& lhs,
                                 const BoolWrapperType& rhs) {
  return !operator==(lhs, rhs);
}

template <typename H>
H AbslHashValue(H state, const BoolWrapperType& type) {
...
template <typename H>
H AbslHashValue(H state, BoolWrapperTypeView type) {
  return H::combine(std::move(state), type.kind());
}
