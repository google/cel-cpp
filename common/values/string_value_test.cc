...
TEST(StringValue, ConvertToJson) {
  EXPECT_THAT(StringValue("foo").ConvertToJson(),
...
TEST(StringValueView, ConvertToJson) {
  EXPECT_THAT(StringValueView("foo").ConvertToJson(),
              IsOkAndHolds(Json(JsonString("foo"))));
}
