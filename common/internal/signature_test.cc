...
TEST_P(TypeSignatureTest, TypeSignature) {
  ...
  std::string signature = common_internal::MakeTypeSignature(param.type);
  EXPECT_EQ(signature, param.expected_signature);
}
...
TEST_P(OverloadSignatureTest, OverloadSignature) {
  ...
  std::string signature = common_internal::MakeOverloadSignature(
      param.function_name, param.args, param.is_member);
  EXPECT_EQ(signature, param.expected_signature);
}
...
std::vector<OverloadSignatureTestCase> GetOverloadSignatureTestCases() {
  return {
      ...
      },
      {
          ...
          .is_member = true,
          .expected_signature =
              "error.hello()",  // Member function without a target
      },
      {
          .args = {StringType{}},
          .is_member = true,
