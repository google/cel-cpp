...
//
// `To` must be a plain non-union class type that is not qualified. If
// `CastTraits` is not specialized, this requires that the static member
// function `bool To::ClassOf(const From&)` exist.
//
...
// which verifies `From` is-a `To`. In non-debug builds, invalid casts are
// undefined behavior.
//
