...
template <typename AstNode, typename ExprType>
...
template <class AstNode, typename ExprType>
struct NavigableAstNodeData {
  AstNode* parent;
  const ExprType* expr;
  ChildKind parent_relation;
  NodeKind node_kind;
  const NavigableAstMetadata<AstNode, ExprType>* absl_nonnull metadata;
  size_t index;
  size_t tree_size;
  size_t height;
  int child_index;
...
struct NavigableAstMetadata {
  ...
  absl::flat_hash_map<const ExprType*, const AstNode* absl_nonnull>
      expr_to_node;
};
...
template <class Derived, typename ExprType>
class NavigableAstNodeBase {
 private:
  using MetadataType = NavigableAstMetadata<Derived, ExprType>;
  using NodeDataType = NavigableAstNodeData<Derived, ExprType>;
...
template <class Derived, class AstNode, typename ExprType>
class NavigableAstBase {
 ...
 private:
  ...
  using MetadataType = NavigableAstMetadata<AstNode, ExprType>;
