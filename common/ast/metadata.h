...
class ExtensionSpecifier {
 public:
  ...
  static const ExtensionSpecifier& DefaultInstance();

  ExtensionSpecifier() = default;
  ExtensionSpecifier(std::string id, std::unique_ptr<Version> version,
                     std::vector<Component> affected_components)
      : id_(std::move(id)),
        affected_components_(std::move(affected_components)),
        version_(std::move(version)) {}

  ExtensionSpecifier(const ExtensionSpecifier& other);
  ExtensionSpecifier(ExtensionSpecifier&& other) = default;
  ExtensionSpecifier& operator=(const ExtensionSpecifier& other);
  ExtensionSpecifier& operator=(ExtensionSpecifier&& other) = default;
  ...
  bool operator==(const ExtensionSpecifier& other) const {
...
class SourceInfo {
 public:
  ...
  SourceInfo(std::string syntax_version, std::string location,
             ...
             std::vector<ExtensionSpecifier> extensions)
  ...
  const std::vector<ExtensionSpecifier>& extensions() const {
    return extensions_;
  }

  std::vector<ExtensionSpecifier>& mutable_extensions() { return extensions_; }
 ...
 private:
  ...
  std::vector<ExtensionSpecifier> extensions_;
};
...
//   they will be named.
enum class WellKnownTypeSpecifier {
  // Unspecified type.
...
class ListTypeSpecifier {
 public:
  ListTypeSpecifier() = default;

  ListTypeSpecifier(const ListTypeSpecifier& rhs)
      : elem_type_(std::make_unique<TypeSpecifier>(rhs.elem_type())) {}
  ListTypeSpecifier& operator=(const ListTypeSpecifier& rhs) {
    elem_type_ = std::make_unique<TypeSpecifier>(rhs.elem_type());
    return *this;
  }
  ListTypeSpecifier(ListTypeSpecifier&& rhs) = default;
  ListTypeSpecifier& operator=(ListTypeSpecifier&& rhs) = default;

  explicit ListTypeSpecifier(std::unique_ptr<TypeSpecifier> elem_type)
      : elem_type_(std::move(elem_type)) {}

  void set_elem_type(std::unique_ptr<TypeSpecifier> elem_type) {
    elem_type_ = std::move(elem_type);
  }

  bool has_elem_type() const { return elem_type_ != nullptr; }

  const TypeSpecifier& elem_type() const;

  TypeSpecifier& mutable_elem_type() {
    if (elem_type_ == nullptr) {
      elem_type_ = std::make_unique<TypeSpecifier>();
    }
    return *elem_type_;
  }

  bool operator==(const ListTypeSpecifier& other) const;

 private:
  std::unique_ptr<TypeSpecifier> elem_type_;
};

// Map type specifier with parameterized key and value types, e.g.
// `map<string, int>`.
class MapTypeSpecifier {
 public:
  MapTypeSpecifier() = default;
  MapTypeSpecifier(std::unique_ptr<TypeSpecifier> key_type,
                   std::unique_ptr<TypeSpecifier> value_type)
      : key_type_(std::move(key_type)), value_type_(std::move(value_type)) {}

  MapTypeSpecifier(const MapTypeSpecifier& rhs)
      : key_type_(std::make_unique<TypeSpecifier>(rhs.key_type())),
        value_type_(std::make_unique<TypeSpecifier>(rhs.value_type())) {}
  MapTypeSpecifier& operator=(const MapTypeSpecifier& rhs) {
    key_type_ = std::make_unique<TypeSpecifier>(rhs.key_type());
    value_type_ = std::make_unique<TypeSpecifier>(rhs.value_type());

    return *this;
  }
  MapTypeSpecifier(MapTypeSpecifier&& rhs) = default;
  MapTypeSpecifier& operator=(MapTypeSpecifier&& rhs) = default;

  void set_key_type(std::unique_ptr<TypeSpecifier> key_type) {
    key_type_ = std::move(key_type);
  }

  void set_value_type(std::unique_ptr<TypeSpecifier> value_type) {
  ...
  const TypeSpecifier& key_type() const;

  const TypeSpecifier& value_type() const;

  bool operator==(const MapTypeSpecifier& other) const;

  TypeSpecifier& mutable_key_type() {
    if (key_type_ == nullptr) {
      key_type_ = std::make_unique<TypeSpecifier>();
    }
    return *key_type_;
  }

  TypeSpecifier& mutable_value_type() {
    if (value_type_ == nullptr) {
      value_type_ = std::make_unique<TypeSpecifier>();
    }
    return *value_type_;
  }
 ...
 private:
  // The type of the key.
  std::unique_ptr<TypeSpecifier> key_type_;

  // The type of the value.
  std::unique_ptr<TypeSpecifier> value_type_;
};
...
class FunctionTypeSpecifier {
 public:
  FunctionTypeSpecifier() = default;
  FunctionTypeSpecifier(std::unique_ptr<TypeSpecifier> result_type,
                        std::vector<TypeSpecifier> arg_types);

  FunctionTypeSpecifier(const FunctionTypeSpecifier& other);
  FunctionTypeSpecifier& operator=(const FunctionTypeSpecifier& other);
  FunctionTypeSpecifier(FunctionTypeSpecifier&&) = default;
  FunctionTypeSpecifier& operator=(FunctionTypeSpecifier&&) = default;

  void set_result_type(std::unique_ptr<TypeSpecifier> result_type) {
    result_type_ = std::move(result_type);
  }

  void set_arg_types(std::vector<TypeSpecifier> arg_types);

  bool has_result_type() const { return result_type_ != nullptr; }

  const TypeSpecifier& result_type() const;

  TypeSpecifier& mutable_result_type() {
    if (result_type_ == nullptr) {
      result_type_ = std::make_unique<TypeSpecifier>();
    }
    return *result_type_;
  }

  const std::vector<TypeSpecifier>& arg_types() const { return arg_types_; }

  std::vector<TypeSpecifier>& mutable_arg_types() { return arg_types_; }

  bool operator==(const FunctionTypeSpecifier& other) const;

 private:
  // Result type of the function.
  std::unique_ptr<TypeSpecifier> result_type_;

  // Argument types of the function.
  std::vector<TypeSpecifier> arg_types_;
};
...
class AbstractType {
 public:
  ...
  AbstractType(std::string name, std::vector<TypeSpecifier> parameter_types);

  void set_name(std::string name) { name_ = std::move(name); }

  void set_parameter_types(std::vector<TypeSpecifier> parameter_types);

  const std::string& name() const { return name_; }

  const std::vector<TypeSpecifier>& parameter_types() const {
    return parameter_types_;
  }

  std::vector<TypeSpecifier>& mutable_parameter_types() {
    return parameter_types_;
  }
 ...
 private:
  ...
  std::vector<TypeSpecifier> parameter_types_;
};
...
class MessageTypeSpecifier {
 public:
  MessageTypeSpecifier() = default;
  explicit MessageTypeSpecifier(std::string type) : type_(std::move(type)) {}

  void set_type(std::string type) { type_ = std::move(type); }

  const std::string& type() const { return type_; }

  bool operator==(const MessageTypeSpecifier& other) const {
    return type_ == other.type_;
  }
...
// TypeSpecifier param type.
//
// The `type_param` string specifies the type parameter name, e.g. `list<E>`
// would be a `list_type` whose element type was a `type_param` type
// named `E`.
class ParamTypeSpecifier {
 public:
  ParamTypeSpecifier() = default;
  explicit ParamTypeSpecifier(std::string type) : type_(std::move(type)) {}

  void set_type(std::string type) { type_ = std::move(type); }

  const std::string& type() const { return type_; }

  bool operator==(const ParamTypeSpecifier& other) const {
    return type_ == other.type_;
  }
...
enum class ErrorTypeSpecifier { kValue = 0 };

using UnsetTypeSpecifier = absl::monostate;

struct DynTypeSpecifier {};

inline bool operator==(const DynTypeSpecifier&, const DynTypeSpecifier&) {
  return true;
}
inline bool operator!=(const DynTypeSpecifier&, const DynTypeSpecifier&) {
  return false;
...
struct NullTypeSpecifier {};
inline bool operator==(const NullTypeSpecifier&, const NullTypeSpecifier&) {
  return true;
}
inline bool operator!=(const NullTypeSpecifier&, const NullTypeSpecifier&) {
  return false;
...
using TypeSpecifierKind =
    absl::variant<UnsetTypeSpecifier, DynTypeSpecifier, NullTypeSpecifier,
                  PrimitiveType, PrimitiveTypeWrapper, WellKnownTypeSpecifier,
                  ListTypeSpecifier, MapTypeSpecifier, FunctionTypeSpecifier,
                  MessageTypeSpecifier, ParamTypeSpecifier,
                  absl_nullable std::unique_ptr<TypeSpecifier>,
                  ErrorTypeSpecifier, AbstractType>;
...
class TypeSpecifier {
 public:
  ...
  explicit TypeSpecifier(TypeSpecifierKind type_kind)
  ...
  TypeSpecifier& operator=(TypeSpecifier&&) = default;

  void set_type_kind(TypeSpecifierKind type_kind) {
    type_kind_ = std::move(type_kind);
  }

  const TypeSpecifierKind& type_kind() const { return type_kind_; }

  TypeSpecifierKind& mutable_type_kind() { return type_kind_; }

  bool has_dyn() const {
    return absl::holds_alternative<DynTypeSpecifier>(type_kind_);
  }

  bool has_null() const {
    return absl::holds_alternative<NullTypeSpecifier>(type_kind_);
  }
  ...
  bool has_well_known() const {
    return absl::holds_alternative<WellKnownTypeSpecifier>(type_kind_);
  }

  bool has_list_type() const {
    return absl::holds_alternative<ListTypeSpecifier>(type_kind_);
  }

  bool has_map_type() const {
    return absl::holds_alternative<MapTypeSpecifier>(type_kind_);
  }

  bool has_function() const {
    return absl::holds_alternative<FunctionTypeSpecifier>(type_kind_);
  }

  bool has_message_type() const {
    return absl::holds_alternative<MessageTypeSpecifier>(type_kind_);
  }

  bool has_type_param() const {
    return absl::holds_alternative<ParamTypeSpecifier>(type_kind_);
  }

  bool has_type() const {
    return absl::holds_alternative<std::unique_ptr<TypeSpecifier>>(type_kind_);
  }

  bool has_error() const {
    return absl::holds_alternative<ErrorTypeSpecifier>(type_kind_);
  }
  ...
  NullTypeSpecifier null() const {
    auto* value = absl::get_if<NullTypeSpecifier>(&type_kind_);
    if (value != nullptr) {
      return *value;
    }
    return {};
  ...
  WellKnownTypeSpecifier well_known() const {
    auto* value = absl::get_if<WellKnownTypeSpecifier>(&type_kind_);
    if (value != nullptr) {
      return *value;
    }
    return WellKnownTypeSpecifier::kWellKnownTypeUnspecified;
  }

  const ListTypeSpecifier& list_type() const {
    auto* value = absl::get_if<ListTypeSpecifier>(&type_kind_);
    if (value != nullptr) {
      return *value;
    }
    static const ListTypeSpecifier* default_list_type = new ListTypeSpecifier();
    return *default_list_type;
  }

  const MapTypeSpecifier& map_type() const {
    auto* value = absl::get_if<MapTypeSpecifier>(&type_kind_);
    if (value != nullptr) {
      return *value;
    }
    static const MapTypeSpecifier* default_map_type = new MapTypeSpecifier();
    return *default_map_type;
  }

  const FunctionTypeSpecifier& function() const {
    auto* value = absl::get_if<FunctionTypeSpecifier>(&type_kind_);
    if (value != nullptr) {
      return *value;
    }
    static const FunctionTypeSpecifier* default_function_type =
        new FunctionTypeSpecifier();
    return *default_function_type;
  }

  const MessageTypeSpecifier& message_type() const {
    auto* value = absl::get_if<MessageTypeSpecifier>(&type_kind_);
    if (value != nullptr) {
      return *value;
    }
    static const MessageTypeSpecifier* default_message_type =
        new MessageTypeSpecifier();
    return *default_message_type;
  }

  const ParamTypeSpecifier& type_param() const {
    auto* value = absl::get_if<ParamTypeSpecifier>(&type_kind_);
    if (value != nullptr) {
      return *value;
    }
    static const ParamTypeSpecifier* default_param_type =
        new ParamTypeSpecifier();
    return *default_param_type;
  }

  const TypeSpecifier& type() const;

  ErrorTypeSpecifier error_type() const {
    auto* value = absl::get_if<ErrorTypeSpecifier>(&type_kind_);
    if (value != nullptr) {
      return *value;
    }
    return ErrorTypeSpecifier::kValue;
  }
  ...
  bool operator==(const TypeSpecifier& other) const {
    if (absl::holds_alternative<std::unique_ptr<TypeSpecifier>>(type_kind_) &&
        absl::holds_alternative<std::unique_ptr<TypeSpecifier>>(
            other.type_kind_)) {
      const auto& self_type =
          absl::get<std::unique_ptr<TypeSpecifier>>(type_kind_);
      const auto& other_type =
          absl::get<std::unique_ptr<TypeSpecifier>>(other.type_kind_);
...
inline FunctionTypeSpecifier::FunctionTypeSpecifier(
    std::unique_ptr<TypeSpecifier> result_type,
    std::vector<TypeSpecifier> arg_types)
    : result_type_(std::move(result_type)), arg_types_(std::move(arg_types)) {}

inline void FunctionTypeSpecifier::set_arg_types(
    std::vector<TypeSpecifier> arg_types) {
