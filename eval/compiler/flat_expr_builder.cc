...
class FlatExprVisitor : public cel::AstVisitor {
 public:
  ...
  void SetPlanRecursiveProgram() { plan_recursive_program_ = true; }

  bool PlanRecursiveProgram() const { return plan_recursive_program_; }
  ...
  absl::optional<int> RecursionEligible() {
    if (!plan_recursive_program_ || program_builder_.current() == nullptr) {
      return absl::nullopt;
    }
 ...
 private:
  ...
  bool plan_recursive_program_ = false;
};
...
absl::StatusOr<FlatExpression> FlatExprBuilder::CreateExpressionImpl(
  ...
  extension_context.set_ast(ast.get());
  ...
  extension_context.clear_navigable_ast();
  ...
  if (options_.max_recursion_depth == -1 || options_.max_recursion_depth > 0) {
    const auto& ast = extension_context.navigable_ast();
    if (!ast) {
      return absl::InternalError("AST is null after transforms");
    }
    if (options_.max_recursion_depth > 0 &&
        ast.Root().height() > options_.max_recursion_depth) {
      if (!options_.enable_recursive_planning_fail_over) {
        return absl::InvalidArgumentError(
            "expression exceeds maximum recursion depth.");
      }
    } else {
      visitor.SetPlanRecursiveProgram();
    }
  }
