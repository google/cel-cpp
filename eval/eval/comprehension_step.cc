...
absl::Status ComprehensionDirectStep::Evaluate1(ExecutionFrameBase& frame,
  ...
  absl::Nonnull<ValueIteratorPtr> range_iter;
  ...
  if (frame.unknown_processing_enabled()) {
    ...
    while (true) {
      ...
      *iter_slot->mutable_attribute() =
          range_attr.Step(AttributeQualifierFromValue(*key));
      ...
      CEL_RETURN_IF_ERROR(loop_step_->Evaluate(
          frame, *accu_slot->mutable_value(), *accu_slot->mutable_attribute()));
    }
  } else {
    while (true) {
      CEL_ASSIGN_OR_RETURN(
