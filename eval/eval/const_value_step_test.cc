...
util::StatusOr<CelValue> RunConstantExpression(const Expr* expr,
                                               const Constant* const_expr,
                                               Arena* arena) {
  auto step_status = CreateConstValueStep(const_expr, expr);
  if (!util::IsOk(step_status)) return step_status.status();
