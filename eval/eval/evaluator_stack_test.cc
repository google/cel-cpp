...
TEST(EvaluatorStackTest, StackPushPop) {
  ...
  ASSERT_EQ(static_cast<cel::IntValue>(stack.Peek()).NativeValue(), 3);
  ASSERT_FALSE(stack.PeekAttribute().empty());
  ASSERT_EQ(stack.PeekAttribute().attribute(), attribute);

  stack.Pop(1);

  ASSERT_EQ(static_cast<cel::IntValue>(stack.Peek()).NativeValue(), 2);
  ASSERT_TRUE(stack.PeekAttribute().empty());

  stack.Pop(1);

  ASSERT_EQ(static_cast<cel::IntValue>(stack.Peek()).NativeValue(), 1);
  ASSERT_TRUE(stack.PeekAttribute().empty());
}
