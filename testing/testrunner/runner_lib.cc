...
absl::StatusOr<CheckedExpr> TestRunner::GetCheckedExpr() const {
  const std::optional<CelExpressionSource>& source =
      test_context_->expression_source();
  if (!source.has_value()) {
  ...
  CheckedExpr checked_expr;
  switch (source->type()) {
    case CelExpressionSource::Type::kCheckedExpr:
      checked_expr = *source->checked_expr();
      break;
    case CelExpressionSource::Type::kRawExpression: {
      CEL_ASSIGN_OR_RETURN(checked_expr,
                           Compile(*source->raw_expression(), *test_context_));
      break;
    }
    case CelExpressionSource::Type::kCelFile: {
      CEL_ASSIGN_OR_RETURN(std::string contents,
                           ReadFileToString(*source->cel_file_path()));
      CEL_ASSIGN_OR_RETURN(checked_expr, Compile(contents, *test_context_));
      break;
    }
  }
