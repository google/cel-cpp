...
absl::StatusOr<CheckedExpr> TestRunner::GetCheckedExpr() const {
  ...
  return std::visit(
      [this](const auto& arg) -> absl::StatusOr<CheckedExpr> {
        using T = std::decay_t<decltype(arg)>;

        if constexpr (std::is_same_v<T, cel::expr::CheckedExpr>) {
          return arg;
        } else if constexpr (std::is_same_v<
                                 T, CelExpressionSource::RawExpression>) {
          return Compile(arg.value, *test_context_);
        } else if constexpr (std::is_same_v<T, CelExpressionSource::CelFile>) {
          CEL_ASSIGN_OR_RETURN(std::string contents,
                               ReadFileToString(arg.path));
