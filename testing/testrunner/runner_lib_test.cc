...
class TestRunnerWithCompilerFixture : public Base {
 public:
  void SetUp() override {
    // Create a compiler.
...
enum class Backend { kRuntime, kBuilder };

class TestRunnerParamTestBase : public ::testing::TestWithParam<Backend> {
 protected:
  absl::StatusOr<std::unique_ptr<CelTestContext>> CreateTestContext(
      CelTestContextOptions options) {
    if (GetParam() == Backend::kRuntime) {
...
class TestRunnerBasicParamTest
    : public TestRunnerWithCompilerFixture<TestRunnerParamTestBase> {};

class TestRunnerParamTest : public TestRunnerParamTestBase {};

using TestRunnerTest = TestRunnerWithCompilerFixture<::testing::Test>;

TEST_P(TestRunnerBasicParamTest, BasicTestReportsSuccess) {
...
TEST_P(TestRunnerBasicParamTest, BasicTestReportsFailure) {
  ASSERT_OK_AND_ASSIGN(cel::ValidationResult validation_result,
...
TEST_P(TestRunnerBasicParamTest, DynamicInputAndOutputReportsSuccess) {
  ASSERT_OK_AND_ASSIGN(cel::ValidationResult validation_result,
...
TEST_P(TestRunnerBasicParamTest, DynamicInputAndOutputReportsFailure) {
  ASSERT_OK_AND_ASSIGN(cel::ValidationResult validation_result,
                       compiler_->Compile("x + y"));
  CheckedExpr checked_expr;
  ASSERT_THAT(cel::AstToCheckedExpr(*validation_result.GetAst(), &checked_expr),
...
INSTANTIATE_TEST_SUITE_P(TestRunnerNoCustomCompilerTest,
                         TestRunnerBasicParamTest,
                         ::testing::Values(Backend::kRuntime,
                                           Backend::kBuilder));
...
INSTANTIATE_TEST_SUITE_P(TestRunnerCustomCompilerTest, TestRunnerParamTest,
                         ::testing::Values(Backend::kRuntime,
                                           Backend::kBuilder));

TEST(TestRunnerCustomCompilerTest, DynamicInputWithoutCompilerFails) {
...
TEST(TestRunnerCustomCompilerTest,
     RunTestFailsWhenNoExpressionSourceIsProvided) {
  const std::string expected_error =
