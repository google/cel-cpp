...
// TODO(uncreated-issue/50): initially, just covering the chained struct select case.
...
absl::optional<SelectInstruction> GetSelectInstruction(
  ...
  if (field_or.has_value()) {
    return SelectInstruction{field_or->number, std::string(field_name)};
  }
  return absl::nullopt;
}
...
class RewriterImpl : public AstRewriterBase {
 ...
 private:
  SelectPath GetSelectPath(Expr* expr) {
    SelectPath result;
    // TODO(uncreated-issue/51): update after C++ runtime ast supports optionals.
    result.opt_index = -1;
