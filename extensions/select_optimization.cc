  ...
  return absl::visit(
      absl::Overload(
          [&](const FieldSpecifier& field_specifier) -> absl::StatusOr<Value> {
            if (!operand.IsStruct()) {
              std::cerr << "operand is not a struct\n"
                        << operand.DebugString() << "\n";
...
absl::StatusOr<Value> OptimizedSelectImpl::ApplySelect(
...

  {
    ...
    if (!presence_test_ && operand_descriptor_ != nullptr &&
      ...
      ProtoSelectStateMachine machine(message, proto_select_path_,
                                      select_path_);
      if (use_unsafe) {
    ...
    auto value_or = struct_value.Qualify(
        select_path_, presence_test_, frame.descriptor_pool(),
        frame.message_factory(), frame.arena());
