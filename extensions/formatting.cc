...
#include "third_party/icu/include/unicode/decimfmt.h"
#include "third_party/icu/include/unicode/errorcode.h"
#include "third_party/icu/include/unicode/locid.h"
#include "third_party/icu/include/unicode/numfmt.h"
...
absl::StatusOr<absl::string_view> FormatFixed(
    ValueManager& value_manager, const Value& value,
    std::optional<int> precision, const icu::Locale& locale,
    std::string& scratch ABSL_ATTRIBUTE_LIFETIME_BOUND);
  ...
  switch (kind) {
    ...
    case ValueKind::kDouble:
      if (value == "NaN") {
        ...
        absl::StrAppend(&target, "\"Infinity\"");
      } else if (value == "-Infinity") {
        absl::StrAppend(&target, "\"-Infinity\"");
      } else {
...
absl::StatusOr<absl::string_view> FormatHex(
    ValueManager& value_manager, const Value& value, bool use_upper_case,
    std::string& scratch ABSL_ATTRIBUTE_LIFETIME_BOUND) {
  switch (value.kind()) {
    case ValueKind::kString:
...
absl::StatusOr<std::unique_ptr<icu::NumberFormat>> CreateDoubleNumberFormater(
    std::optional<int> precision, bool use_scientific_notation,
...
absl::StatusOr<absl::string_view> FormatFixed(
  ...
  CEL_ASSIGN_OR_RETURN(
      auto formatter,
      CreateDoubleNumberFormater(precision, /*use_scientific_notation=*/false,
                                 locale));

  icu::ErrorCode error_code;  // NOLINT
  icu::UnicodeString output;
  formatter->format(number, output, error_code);
...
absl::StatusOr<absl::string_view> FormatScientific(
  ...
  CEL_ASSIGN_OR_RETURN(auto number, GetDouble(value, scratch));
  CEL_ASSIGN_OR_RETURN(
      auto formatter, CreateDoubleNumberFormater(
                          precision, /*use_scientific_notation=*/true, locale));
  ...
  if (error_code.isSuccess()) {
    ...
    return absl::InternalError(absl::StrCat(
        "Failed to format scientific number: ", error_code.errorName()));
  }
}
...
absl::StatusOr<std::pair<int64_t, absl::string_view>> ParseAndFormatClause(
  ...
  switch (format[read]) {
    ...
    case 'd': {
      CEL_ASSIGN_OR_RETURN(auto result,
                           FormatDecimal(value_manager, value, scratch));
      return std::pair{read, result};
    }
    case 'f': {
      CEL_ASSIGN_OR_RETURN(
          auto result,
          FormatFixed(value_manager, value, precision, locale, scratch));
      return std::pair{read, result};
    }
    case 'e': {
      CEL_ASSIGN_OR_RETURN(
          auto result,
          FormatScientific(value_manager, value, precision, locale, scratch));
      return std::pair{read, result};
    }
    case 'b': {
      CEL_ASSIGN_OR_RETURN(auto result,
                           FormatBinary(value_manager, value, scratch));
      return std::pair{read, result};
    }
    case 'x':
    case 'X': {
      CEL_ASSIGN_OR_RETURN(
          auto result,
          FormatHex(value_manager, value,
                    /*use_upper_case=*/format[read] == 'X', scratch));
      return std::pair{read, result};
    }
    case 'o': {
      CEL_ASSIGN_OR_RETURN(auto result,
                           FormatOctal(value_manager, value, scratch));
      return std::pair{read, result};
    }
    default:
      return absl::InvalidArgumentError(absl::StrFormat(
...
absl::Status RegisterStringFormattingFunctions(FunctionRegistry& registry,
  ...
  CEL_RETURN_IF_ERROR(registry.Register(
      VariadicFunctionAdapter<absl::StatusOr<Value>, StringValue, ListValue>::
